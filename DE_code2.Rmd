...
```
# Remove outlier SV5 to see if anything changes
*More differential genes come out after SV5 removal*
This is a loose threshold; so at the end, more significant genes show up....
around 95% got overlapped (tested in threeovelapgenes, fouroverlapgenes, 14weekdbvsPBS) 
...
check on some individual dots to see if the significance is reliable??? check on the raw data and genome browser
...
utr_count$SV5 <- NULL  #!!!remove SV5

#create the design matrix
design_colname <- factor(paste(rep("Sample",20),seq(20),sep=""))
treat <- factor(c(rep("PBS",5),rep("exosomes",15)),levels=c("PBS","exosomes"))
age <- factor(c(rep("NA",5),rep("14week",5),rep("24week",10)),levels=c("NA","14week","24week"))
genotype <- factor(c(rep("NA",5),rep("wt",3),rep("db",2),rep("wt",5),rep("db",5)),levels=c("NA","wt","db"))
...
y <- DGEList(counts=utr_count[,2:21],genes=utr_count[,1],group=group) #!!!change 22 to 21, since remove SV5
...
heatplot(scale(variable.y1[,1:20]),scale="none",labRow=F)
...
# gprofiler(as.character(fouroverlap.upgenes),organism = "hsapiens") ## no enrichment here; 
# gprofiler(as.character(fouroverlap.downgenes),organism = "hsapiens") ## similarly, metal ion pathways
# 
# gprofiler(as.character(threeoverlap.upgenes),organism = "hsapiens") # 
# gprofiler(as.character(threeoverlap.downgenes),organism = "hsapiens") #
# 
# gprofiler(as.character(week24dbvswt.upgenes),organism = "hsapiens") ## 
# gprofiler(as.character(week24dbvswt.downgenes),organism = "hsapiens") # still broad and hard to interpret  
```
